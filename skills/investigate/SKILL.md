...
Determinar qué modo aplicar según `$ARGUMENTS`:

1. **Leer `$ARGUMENTS`**: ¿es un archivo `.md` que existe en el working directory?
   - **SÍ** → **MODO SEGUIMIENTO** (ir a "Seguimiento de investigación existente")
   - **NO** → **MODO CREAR** (ir a "Crear investigación nueva")

---

# MODO CREAR — Investigación nueva sobre $ARGUMENTS

Vas a conducir una investigación estructurada sobre **$ARGUMENTS** usando el flujo de 5 fases. El resultado es un documento markdown con trazabilidad lógica completa.
...
El argumento `$ARGUMENTS` es un archivo de investigación existente. Leer el documento, detectar estado, y actuar.
